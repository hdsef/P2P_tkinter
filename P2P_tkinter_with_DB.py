...
cl_ch = 0 #close chat
number=0
def put_user_into_db(Ln,Ip):
    query = "INSERT INTO users(id,Login,IP) " \
            "VALUES(NULL,%s,%s)"
    args = (Ln, Ip)
    cursor.execute(query, args)
    conn.commit()

def put_message_into_db(Ln,Dest,Mess):
    query = "INSERT INTO messages(users_id,Login,Destination, Message) " \
            "VALUES(NULL,%s,%s,%s)"
    args = (Ln, Dest, Mess)
    cursor.execute(query, args)
    conn.commit()
...
def notification(event):  
    ...
    put_user_into_db(str(login.get()), IPaddr)
    users.append(LogIp)
        ...
        if data.decode().find("@#") != -1:
            ...
            if existing_user != 1:
                ...
                if data.decode() != LogIp:
                    sock.sendto(LogIp.encode(), (writeback[0], UDP_PORT)) #����� ������ ������������ ���������� ����� � IP
...
def Sending(event):
    ...
    while i < len(users):
        ...
        if users_list.get(number) == IP[1]:
            events.clear()    #event=False ������������� Receiving
            a = (str(login.get()) + "|" + message).encode()
            sock.sendto(a, (IP[0], UDP_PORT))
            put_message_into_db(str(login.get()),IP[1],message)
            with open("chat" + IP[1] + ".txt", "a") as chat:
                chat.write("\n" + a.decode())
            events.set()    #event=True ��������� Receiving ���������� ������
        i += 1
...
label2 = Label(text="Insert login", justify=LEFT)
